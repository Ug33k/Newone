...
import { Task, CreateTaskPayload, UpdateTaskPayload, SerializedTask, KanbanStatus, EisenhowerQuadrant } from '../types/task';
...
export const useTaskStore = create<TaskStore>((set, get) => {
  ...
  return {
    ...
    getTasksByQuadrant: (quadrant: EisenhowerQuadrant) => {
      return get().tasks.filter((t) => t.quadrant === quadrant).sort((a, b) => a.order - b.order);
    },

    getTasksByStatus: (status: KanbanStatus) => {
      return get().tasks.filter((t) => t.kanbanStatus === status).sort((a, b) => a.order - b.order);
    },
